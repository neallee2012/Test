...
object Transpose {
  ...
  def outputSchema(sc: SparkContext, outputDir: String, metaDir: String, schema1: String, byColPos: Array[Int], valColPos: Int, byColsPosMap: Map[String, Int], keyPos: List[Int], keepNonTrCols: Boolean, prefixName: String, transposeColSchema: Map[String, String]) = {
    ...
    val meta1List = if (keepNonTrCols) JoinData.remove(meta1, dropbyCol_byVal.sorted.toList).toList
    else {
      for (i <- 0 to keyPos.size - 1) yield (meta1(keyPos(i)))
    }
    //val meta1 =  schema1.split("~\\|")
  ...
  def processType(sample: Array[(String, String)], byColsPosMap: Map[String, String]) = {
    ...
    sample.foreach(in => {
      ...
      if (typeLen._1 != "" || typeLen._1 != null) {
        //if empty keep on next sample row
        typeLen._1 match {
  ...
  def main(args: Array[String]) {
    ...
    val byColsPosMap = distinctbyCols.foldLeft(scala.collection.mutable.LinkedHashMap[String, Int]())((b, a) => {
        b += (a -> b.size); b
      })
